...
const Evaluation = () => {
	...
	const questions = Object.entries(categories).flatMap(([key, qs]) => qs.map((q) => ({ category: key, question: q })));
  ...
  // for uploading test results to the server
							...
							{Object.entries(getCategoryScores()).map(([key, score]) => {
								...
								return (
									<motion.div
										...
										<strong>{key.charAt(0).toUpperCase() + key.slice(1)}:</strong> {status} ({score}/{max})
									</motion.div>
								);
							})}
