...
const Evaluation = () => {
  ...
  const questions = Object.entries(categories).flatMap(([category, items]) =>
    items.map(({ q, scale, reverse }: { q: string; scale: number; reverse?: boolean }) => ({ category, question: q, scale, reverse: reverse || false }))
  );
  ...
  useEffect(() => {
    if (showResults) {
      ...
      const hasSevere = Object.entries(scores).some(([key, score]) =>
        score >= categories[key].length * (categories[key][0].scale * 0.75)
      );
  ...
  return (
    <div className={styles.evaluationContainer}>
      <FloatingChatbot isOpen={chatbotOpen} onToggle={() => setChatbotOpen(prev => !prev)} mode="evaluation" hoveredSection={null} />
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className={styles.evaluationCard}>
        {showResults ? (
          <>
            ...
            <div className={styles.resultText}>
              ...
              {Object.entries(getCategoryScores()).map(([key, score]) => {
                ...
                return (
                  <motion.div key={key} className={styles.resultBox} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                    <strong>{formatCategory(key)}:</strong> {status} ({score}/{max})
                  </motion.div>
                );
              })}
              {Object.entries(getCategoryScores()).some(([key, score]) => score >= categories[key].length * (categories[key][0].scale * 0.75)) && (
                <motion.div className={styles.severeWarning} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.5 }}>
                  <p>One or more categories show high or severe risk. We recommend seeking help.</p>
                  <a href="https://www.google.com/maps/search/mental+health+professionals+near+me" target="_blank" rel="noopener noreferrer" className={styles.getHelpLink}>
                    Find a Professional Near You
                  </a>
                </motion.div>
              )}
            </div>
            <motion.button onClick={() => { setShowResults(false); setStarted(false); setCurrentQuestionIndex(0); setAnswers([]); }} className={styles.restartButton} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
              Restart Evaluation
            </motion.button>
          </>
        ) : started ? (
          <>
            ...
            <p className={styles.progressText}>Question {currentQuestionIndex + 1} of {questions.length}</p>
            <div className={styles.optionsContainer}>
              {[...Array(questions[currentQuestionIndex].scale + 1).keys()].map((index) => (
                <motion.button key={index} onClick={() => handleOptionSelect(index)} className={styles.optionButton} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          ...
          <>
            <motion.button onClick={() => navigate("/dashboard")} className={styles.backButton} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
            ...
            <p className={styles.description}>This test includes questions for anxiety, stress, insomnia, depression, and self-esteem. Be honest for accurate insights.</p>
            <motion.button onClick={() => setStarted(true)} className={styles.startButton} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
              Start Evaluation
            </motion.button>
          </>
        )}
      ...
      {!feedbackGiven && (
        <div className="mt-6 p-4 bg-white rounded-xl shadow-lg max-w-md mx-auto text-center">
          ...
          <div className="flex justify-center gap-4">
            <button onClick={() => { const mood = (localStorage.getItem("todayMood") || "unknown"); logFeedback(mood, "evaluation", 1); setFeedbackGiven(true); }} className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded">
              Yes
            </button>
            <button onClick={() => { const mood = (localStorage.getItem("todayMood") || "unknown"); logFeedback(mood, "evaluation", 0); setFeedbackGiven(true); }} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded">
              No
            </button>
          </div>
        </div>
